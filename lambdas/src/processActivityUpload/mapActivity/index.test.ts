...
describe('mapActivity()', () => {
  test('should work', async () => {
    const file = fs.readFileSync(
      path.resolve(path.resolve('samples'), 'lots-of-points.fit')
    );

    const result = await mapFileToActivity('lots-of-points.fit', file);
  ...
  test('should map an gpx file with GPS data to an activity', async () => {
    const eventFile = fs.readFileSync(
      path.resolve(path.resolve('samples'), 'gpx-with-points.gpx')
    );
  ...
  test('should map an tcx file with GPS data to an activity', async () => {
    const file = fs.readFileSync(
      path.resolve(path.resolve('samples'), 'tcx-with-points.tcx')
    );
  ...
  test('should not map an gpx file with no GPS data', async () => {
    const file = fs.readFileSync(
      path.resolve(path.resolve('samples'), 'no-points.gpx')
    );
  ...
  test('should map an tcx file with no GPS data', async () => {
    const file = fs.readFileSync(
      path.resolve(path.resolve('samples'), 'tcx-no-points.tcx')
    );
  ...
  test('should map an fit file with GPS data', async () => {
    const file = fs.readFileSync(
      path.resolve(path.resolve('samples'), 'fit-with-points.fit'),
      null
    );
  ...
  test('should map an fit file with no GPS data', async () => {
    const file = fs.readFileSync(
      path.resolve(path.resolve('samples'), 'fit-no-points.fit'),
      null
    );
...
describe('generateTitle()', () => {
  [
    {
      date: new Date('2020-01-12T19:52:33Z'),
      type: 'Running',
    ...
    },
    {
      date: new Date('2020-01-12T19:52:33Z'),
      type: 'Hiking',
      expected: 'Evening Hike',
    },
    {
      date: new Date('2020-01-12T19:52:33'),
      type: 'Running',
      expected: 'Evening Run',
    },
  ].forEach((testCase) => {
