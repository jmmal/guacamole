...
const mapFileToActivity = async (
  ...
  try {
    if (filename.match('.fit$')) {
      ...
      event = await SportsLib.importFromTCX(
        new DOMParser().parseFromString(filestring, 'application/xml')
      );
    } else {
      return null;
    }
  ...
  const positionData = activity.hasPositionData()
  ...
  return {
    ...
    title:
      activity.name === ''
        ? generateTitle(activity.startDate, activity.type)
        : activity.name,
    type: activity.type,
